...
func BypassAddressAtBlock(blockNumber int64) (string, bool) {
	if addr, exists := bypassBlockMap[blockNumber]; exists {
		return addr, true
	}
	return "", false
}

func BypassAddressBal(addr string) (string, bool) {
	if bal, exists := bypassAddrMap[addr]; exists {
		return bal, true
	}
	return "", false
}
