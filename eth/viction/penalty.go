...
func PenalizeValidatorsTIPSigning(c *posv.Posv, config *chain.Config, posvConfig *chain.PosvConfig, vicConfig *chain.VictionConfig,
	...
	if config.IsTIPRandomize(blockNumber) {
		return penalties, nil
	}
	return comebacks, nil
}

func PenalizeValidatorsDefault(c *posv.Posv, config *chain.Config, posvConfig *chain.PosvConfig, vicConfig *chain.VictionConfig,
	header *types.Header,
	chain consensus.ChainReader,
) ([]common.Address, error) {
	blockNumber := header.Number.Uint64()
