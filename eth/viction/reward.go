...
func CalcRewardsForValidators(
	c *posv.Posv, config *chain.Config, posvConfig *chain.PosvConfig, vicConfig *chain.VictionConfig,
	...
	for i := prevCheckpoint + (posvConfig.Epoch * 2) - 1; i >= startBlockNumber; i-- {
		...
		for _, tx := range signData {
			signedBlockHash := common.BytesToHash(tx.GetData()[len(tx.GetData())-32:])
			signer, _ := tx.GetSender()
	...
	for i := startBlockNumber; i <= endBlockNumber; i++ {
		if i%vicConfig.ValidatorSignInterval == 0 || !config.IsTIP2019(i) {
...
func CalcRewardsForStakeholders(c *posv.Posv, config *chain.Config, posvConfig *chain.PosvConfig, vicConfig *chain.VictionConfig,
	header *types.Header, validatorRewards map[common.Address]*posv.ValidatorReward, state *state.IntraBlockState, logger log.Logger,
	...
	for v, vr := range validatorRewards {
		...
		if len(voters) > 0 {
			...
			for _, voter := range voters {
				if _, ok := voterCaps[voter]; ok && !config.IsTIP2019(blockNumber) {
					continue
				}
